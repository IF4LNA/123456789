...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pastikan elemen ditemukan sebelum memasang fungsi
    if (toggle && message) {
        ...
        console.log("Toggle System Ready!"); // Cek di console (F12)
    } else {
        console.error("Elemen Toggle atau Message tidak ditemukan!");
    }
});
...
function toggleAudio() {
    if (audio.paused) {
        audio.play();
        icon.innerHTML = "⏸️"; // Ganti icon jadi pause
        audio.volume = 0.5;    // Atur volume agar tidak terlalu keras
    } else {
        audio.pause();
        icon.innerHTML = "🎵"; // Ganti icon jadi musik kembali
    }
}

// Opsional: Musik mulai otomatis saat user pertama kali menyentuh layar
